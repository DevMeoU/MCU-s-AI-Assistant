...
class AudioConverterApp:
    def __init__(self, master):
        self.master = master
        master.title("Audio/P3 Batch Conversion Tool / Công cụ chuyển đổi hàng loạt âm thanh/P3")
        master.geometry("680x600")  # Adjust window height / Điều chỉnh chiều cao cửa sổ
        ...
        # Create UI components / Tạo UI component
        self.create_widgets()
        self.redirect_output()

    def create_widgets(self):
        # Mode selection / Chọn chế độ
        mode_frame = ttk.LabelFrame(self.master, text="Conversion Mode / Chế độ chuyển đổi")
        mode_frame.grid(row=0, column=0, padx=10, pady=5, sticky="ew")
        
        ttk.Radiobutton(mode_frame, text="Audio to P3 / Âm thanh sang P3", variable=self.mode,
                        value="audio_to_p3", command=self.toggle_settings,
                        width=12).grid(row=0, column=0, padx=5)
        ttk.Radiobutton(mode_frame, text="P3 to Audio / P3 sang Âm thanh", variable=self.mode,
                        value="p3_to_audio", command=self.toggle_settings,
                        width=12).grid(row=0, column=1, padx=5)

        # Loudness settings / Cài đặt độ ồn
        self.loudnorm_frame = ttk.Frame(self.master)
        self.loudnorm_frame.grid(row=1, column=0, padx=10, pady=5, sticky="ew")
        
        ttk.Checkbutton(self.loudnorm_frame, text="Enable Loudness Adjustment / Bật điều chỉnh độ ồn", 
        ...
        # File selection / Chọn tệp
        file_frame = ttk.LabelFrame(self.master, text="Input Files / Tệp đầu vào")
        file_frame.grid(row=2, column=0, padx=10, pady=5, sticky="nsew")
        
        # File operation buttons / Nút thao tác tệp
        ttk.Button(file_frame, text="Select Files / Chọn tệp", command=self.select_files,
                  width=12).grid(row=0, column=0, padx=5, pady=2)
        ttk.Button(file_frame, text="Remove Selected / Xóa đã chọn", command=self.remove_selected,
                  width=12).grid(row=0, column=1, padx=5, pady=2)
        ttk.Button(file_frame, text="Clear List / Xóa danh sách", command=self.clear_files,
                  width=12).grid(row=0, column=2, padx=5, pady=2)

        # File list (using Treeview) / Danh sách tệp (sử dụng Treeview)
        self.tree = ttk.Treeview(file_frame, columns=("selected", "filename"), 
                               show="headings", height=8)
        self.tree.heading("selected", text="Selected / Đã chọn", anchor=tk.W)
        self.tree.heading("filename", text="Filename / Tên tệp", anchor=tk.W)
        ...
        output_frame = ttk.LabelFrame(self.master, text="Output Directory / Thư mục đầu ra")
        ...
        ttk.Button(output_frame, text="Browse / Duyệt", command=self.select_output_dir,
                  width=8).grid(row=0, column=1, padx=5)

        # Conversion button area / Khu vực nút chuyển đổi
        button_frame = ttk.Frame(self.master)
        button_frame.grid(row=4, column=0, padx=10, pady=10, sticky="ew")
        
        ttk.Button(button_frame, text="Convert All Files / Chuyển đổi tất cả tệp", command=lambda: self.start_conversion(True),
                  width=15).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Convert Selected Files / Chuyển đổi tệp đã chọn", command=lambda: self.start_conversion(False),
                  width=15).pack(side=tk.LEFT, padx=5)

        # Log area / Khu vực nhật ký
        log_frame = ttk.LabelFrame(self.master, text="Log / Nhật ký")
        log_frame.grid(row=5, column=0, padx=10, pady=5, sticky="nsew")
        
        self.log_text = tk.Text(log_frame, height=14, width=80)
        self.log_text.pack(fill=tk.BOTH, expand=True)

        # Configure layout weights / Cấu hình trọng số bố cục
    ...
    def on_tree_click(self, event):
        """Handle checkbox click event / Xử lý sự kiện nhấp vào hộp kiểm"""
        region = self.tree.identify("region", event.x, event.y)
        if region == "cell":
            col = self.tree.identify_column(event.x)
            item = self.tree.identify_row(event.y)
            if col == "#1":  # Clicked on the selected column / Đã nhấp vào cột đã chọn
                current_val = self.tree.item(item, "values")[0]
                new_val = "[√]" if current_val == "[ ]" else "[ ]"
                self.tree.item(item, values=(new_val, self.tree.item(item, "values")[1]))

    def remove_selected(self):
        """Remove selected files / Xóa các tệp đã chọn"""
        to_remove = []
    ...
    def start_conversion(self, convert_all):
        ...
        if not input_files:
            ...
            messagebox.showwarning("Warning / Cảnh báo", msg)
            return
        ...
        try:
            ...
            print(f"Conversion initialization failed: {str(e)}") # Chuyển đổi khởi tạo thất bại

    def convert_audio_to_p3(self, target_lufs, input_files):
        """Audio to P3 conversion logic / Logic chuyển đổi âm thanh sang P3"""
        for input_path in input_files:
            try:
                ...
                print(f"Converting: {filename}") # Đang chuyển đổi
    ...
    def convert_p3_to_audio(self, input_files):
        """P3 to Audio conversion logic / Logic chuyển đổi P3 sang Âm thanh"""
        for input_path in input_files:
            try:
                filename = os.path.basename(input_path)
                base_name = os.path.splitext(filename)[0]
