...
class P3PlayerApp:
    ...
    def add_file(self):
        files = filedialog.askopenfilenames(filetypes=[("P3 File", "*.p3")])
        if files:
    ...
    def update_playlist(self):
        ...
        for file in self.playlist:
            self.playlist_listbox.insert(tk.END, os.path.basename(file))  # Only display filename

    def update_status(self, status_text, color="blue"):
        """Update content of status label"""
        ...
        while True:
            ...
            if self.current_index >= len(self.playlist):
                if self.loop_playback.get():  # If loop playback is checked
                    self.current_index = 0  # Go back to the first song
                else:
