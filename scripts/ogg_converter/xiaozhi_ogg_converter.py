...
class AudioConverterApp:
    def __init__(self, master):
        self.master = master
        master.title("XiaoZhi AI OGG Audio Batch Converter Tool") # Công cụ chuyển đổi hàng loạt âm thanh OGG AI XiaoZhi
        master.geometry("680x600")  # Adjust window height # Điều chỉnh chiều cao height cửa sổ
        ...
        # Create UI components # Tạo các thành phần UI
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        # Loudness settings # Cài đặt độ lớn
        ...
        self.tree.heading("selected", text="Selected", anchor=tk.W) # Đã chọn
        self.tree.heading("filename", text="Filename", anchor=tk.W) # Tên tệp
    ...
    def clear_files(self):
        """Clear all files""" # Xóa tất cả các tệp
        for item in self.tree.get_children():
            self.tree.delete(item)
    ...
    def start_conversion(self, convert_all):
        ...
        if not input_files:
            msg = "No convertible files found" if convert_all else "No files selected" # Không tìm thấy tệp có thể chuyển đổi / Chưa chọn tệp nào
            messagebox.showwarning("Warning", msg) # Cảnh báo
            return
        ...
        try:
            ...
            print(f"Conversion initialization failed: {str(e)}") # Khởi tạo chuyển đổi thất bại

    def convert_audio_to_ogg(self, target_lufs, input_files):
        """Audio to OGG conversion logic""" # Logic chuyển đổi âm thanh sang OGG
    ...
    def convert_ogg_to_audio(self, input_files):
        """OGG to audio conversion logic""" # Logic chuyển đổi OGG sang âm thanh
        for input_path in input_files:
            try:
                filename = os.path.basename(input_path)
                base_name = os.path.splitext(filename)[0]
