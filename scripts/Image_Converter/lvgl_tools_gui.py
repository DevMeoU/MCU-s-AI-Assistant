...
HELP_TEXT = """LVGL Image Conversion Tool Usage Instructions:

1. Add files: Click the "Add Files" button to select images to convert, supports batch import

2. Remove files: Select the checkbox "[ ]" before the file in the list (it will become "[√]" after selection), click "Remove Selected" to delete the selected files

3. Set resolution: Choose the required resolution, e.g., 128x128
   It is recommended to select based on your device's screen resolution. Too large or too small will affect the display effect.

4. Color format: Selecting "Auto Detect" will automatically choose based on whether the image is transparent, or specify manually
...
class ImageConverterApp:
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell":
            ...
            if col == "#1":  # Clicked on the selection column
        ...
        for idx, file_path in enumerate(input_files):
            try:
                ...
                with Image.open(file_path) as img:
                    ...
                    success_count += 1
                    os.unlink(temp_path)
                    print(f"Conversion successful: {base_name}.c\n")

            except Exception as e:
                print(f"Conversion failed: {str(e)}\n")

        print(f"Conversion complete! Successful {success_count}/{total_files} files\n")
