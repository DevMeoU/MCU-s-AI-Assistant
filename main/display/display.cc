...
void Display::UpdateStatusBar(bool update_all) {
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (low_battery_popup_ != nullptr) {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging) {
                ...
                if (!lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // Nếu hộp cảnh báo pin yếu hiển thị thì ẩn
    ...
    if (update_all || seconds_counter++ % 10 == 0) {
        // Khi nâng cấp firmware, không đọc trạng thái mạng 4G để tránh chiếm tài nguyên UART
...
void Display::SetEmotion(const char* emotion) {
    ...
    // Nếu tìm thấy biểu cảm phù hợp thì hiển thị biểu tượng tương ứng, ngược lại hiển thị biểu cảm neutral mặc định
    if (it != emotions.end()) {
...
void Display::SetMusicInfo(const char* song_name) {
    // Triển khai mặc định: Đối với chế độ không phải WeChat, hiển thị tên bài hát trong nhãn tin nhắn trò chuyện
    DisplayLockGuard lock(this);
    if (chat_message_label_ == nullptr) {
        return;
    }
