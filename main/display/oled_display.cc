...
void OledDisplay::SetupUI_128x64() {
    ...
    // Tạo container bên trái với độ rộng cố định
    content_left_ = lv_obj_create(content_);
    lv_obj_set_size(content_left_, 32, LV_SIZE_CONTENT);  // Độ rộng cố định 32 pixel
    ...
    // Tạo container bên phải có thể mở rộng
    ...
    // Bắt đầu cuộn chữ sau khi delay một khoảng thời gian nhất định
    static lv_anim_t a;
    lv_anim_init(&a);
...
void OledDisplay::SetTheme(const Theme& theme) {
    DisplayLockGuard lock(this);
