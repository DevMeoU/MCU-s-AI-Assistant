...
BoxAudioCodecLite::BoxAudioCodecLite(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    duplex_ = true; // Có phải duplex không
    input_reference_ = input_reference; // Có sử dụng đầu vào tham chiếu để thực hiện triệt tiêu tiếng vang không
    if (input_reference) {
...
void BoxAudioCodecLite::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        // Độ lợi microphone để giải quyết vấn đề thu âm quá nhỏ
...
int BoxAudioCodecLite::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
        ...
        if (input_reference_) { // Bảng không hỗ trợ loopback phần cứng, sử dụng bộ đệm phát lại để thực hiện triệt tiêu tiếng vang
            if (write_pos_ - read_pos_ + samples > ref_buffer_.size()) { 
                assert(ref_buffer_.size() >= samples);
                // Tràn ghi, chỉ giữ lại dữ liệu gần đây nhất
