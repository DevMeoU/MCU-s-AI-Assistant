...
AdcPdmAudioCodec::AdcPdmAudioCodec(int input_sample_rate, int output_sample_rate,
    ...
    if(pdm_speak_n != GPIO_NUM_NC){
        ...
        esp_rom_gpio_connect_out_signal(pdm_speak_n, I2SO_SD_OUT_IDX, 1, 0); // Đảo ngược tín hiệu SD OUT
        gpio_set_drive_capability(pdm_speak_n, GPIO_DRIVE_CAP_0);
    }

    // Khởi tạo bộ hẹn giờ đầu ra
...
void AdcPdmAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        // Buộc cấu hình lại xung nhịp PDM TX theo cấu hình bảng, ghi đè lên up_sample_fs mặc định trong set_fmt của thư viện bên thứ ba
        // Nếu kênh đã được bật, trước tiên hãy tắt, sau đó cấu hình lại, cuối cùng bật lại
        ...
        // Bắt đầu bộ hẹn giờ khi bật đầu ra
        if (output_timer_) {
            esp_timer_start_once(output_timer_, TIMER_TIMEOUT_US);
        }

    } else {
        // Dừng bộ hẹn giờ khi tắt đầu ra
        if (output_timer_) {
            esp_timer_stop(output_timer_);
        }
        if(pa_ctrl_pin_ != GPIO_NUM_NC){
...
// Triển khai hàm callback của bộ hẹn giờ
