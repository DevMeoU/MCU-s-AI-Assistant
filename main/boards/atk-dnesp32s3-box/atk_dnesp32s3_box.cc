...
private:
    ...
    void InitializeI2c() {
        ...
        if (xl9555_in_->GetPingState(0x0020) == 1) {
            es8311_detected_ = true;    /* Cờ thiết bị âm thanh, khi SPK_CTRL_IO ở mức cao, cờ này được đặt thành 1 và xác định là ES8311 */
        } else {
            es8311_detected_ = false;    /* Cờ thiết bị âm thanh, khi SPK_CTRL_IO ở mức thấp, cờ này được đặt thành 0 và xác định là NS4168 */
        }
...
public:
    ...
    virtual AudioCodec* GetAudioCodec() override {
        /* Khởi tạo codec dựa trên kết quả dò tìm */
        if (es8311_detected_) {
