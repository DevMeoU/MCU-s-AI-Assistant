...
private:
    ...
    void CheckBatteryStatus() {
        ...
        // Nếu dữ liệu điện lượng pin không đủ, đọc dữ liệu điện lượng pin
        if (adc_values_.size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData();
            return;
        }

        // Nếu dữ liệu điện lượng pin đủ, đọc dữ liệu điện lượng pin mỗi kBatteryAdcInterval tick
        ticks_++;
    ...
    void ReadBatteryAdcData() {
        ...
        // Định nghĩa các khoảng điện lượng pin
        const struct {
            uint16_t adc;
            uint8_t level;
        } levels[] = {
        ...
        // Khi dưới giá trị tối thiểu
        if (average_adc < levels[0].adc) {
            battery_level_ = 0;
        }
        // Khi trên giá trị tối đa
...
public:
    ...
    bool IsCharging() {
        // Nếu pin đã đầy, không hiển thị trạng thái sạc
