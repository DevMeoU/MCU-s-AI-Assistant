...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Điện áp tối thiểu mà pin có thể xả
    ...
    // Khi pin không được kết nối, trạng thái sạc đọc được không ổn định và battery_voltage đôi khi có thể vượt quá charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
