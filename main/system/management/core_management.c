...
// Thông tin core
static core_info_t core_info[2] = {0};
...
static const BaseType_t task_core_mapping[CORE_TASK_TYPE_MAX] = {
    ...
    [CORE_TASK_TYPE_AUDIO_PROCESSOR] = AUDIO_INPUT_CORE, // Audio processor task trên core 1
    [CORE_TASK_TYPE_WAKE_WORD_DETECTION] = AUDIO_INPUT_CORE, // Wake word detection task trên core 1
    [CORE_TASK_TYPE_WAKE_WORD_ENCODING] = AUDIO_INPUT_CORE,  // Wake word encoding task trên core 1
...
static const UBaseType_t task_priority_mapping[CORE_TASK_TYPE_MAX] = {
    [CORE_TASK_TYPE_WIFI] = 10,                          // Wi-Fi có priority cao
    ...
    [CORE_TASK_TYPE_OTA] = 1,                            // OTA task (low priority)
    [CORE_TASK_TYPE_UI] = DISPLAY_UPDATE_PRIORITY,       // UI task
    [CORE_TASK_TYPE_AUDIO_PROCESSOR] = AUDIO_INPUT_PRIORITY, // Audio processor task (real-time)
    [CORE_TASK_TYPE_WAKE_WORD_DETECTION] = AUDIO_INPUT_PRIORITY, // Wake word detection task (real-time)
    [CORE_TASK_TYPE_WAKE_WORD_ENCODING] = 2,             // Wake word encoding task
    [CORE_TASK_TYPE_BOARD_ACTION] = configMAX_PRIORITIES - 1, // Board action task (high priority)
    [CORE_TASK_TYPE_BATTERY_MONITORING] = 10,            // Battery monitoring task (high priority)
    [CORE_TASK_TYPE_TOUCHPAD] = 5                        // Touchpad handling task
};

// Bảng ánh xạ task type sang stack size
static const uint32_t task_stack_size_mapping[CORE_TASK_TYPE_MAX] = {
    [CORE_TASK_TYPE_WIFI] = 4096,                        // Wi-Fi stack size
    ...
    [CORE_TASK_TYPE_OTA] = 4096,                         // OTA task
    [CORE_TASK_TYPE_UI] = DISPLAY_UPDATE_STACK_SIZE,     // UI task
    [CORE_TASK_TYPE_AUDIO_PROCESSOR] = 4096,             // Audio processor task
    [CORE_TASK_TYPE_WAKE_WORD_DETECTION] = 4096,         // Wake word detection task
    [CORE_TASK_TYPE_WAKE_WORD_ENCODING] = 4096 * 7,      // Wake word encoding task
    [CORE_TASK_TYPE_BOARD_ACTION] = 1024 * 4,            // Board action task
    [CORE_TASK_TYPE_BATTERY_MONITORING] = 1024,          // Battery monitoring task
    [CORE_TASK_TYPE_TOUCHPAD] = 4096                     // Touchpad handling task
};

void core_management_init(void) {
    // Khởi tạo thông tin core
    memset(core_info, 0, sizeof(core_info));
    core_info[0].is_initialized = true;
    core_info[1].is_initialized = true;
...
bool core_management_is_task_allowed_on_core(core_task_type_t task_type, BaseType_t core_id) {
    if (task_type >= CORE_TASK_TYPE_MAX || core_id < 0 || core_id > 1) {
        return false;
    }
