...
private:
    ...
    std::optional<int> min_value_;  // Thêm mới: giá trị nhỏ nhất của số nguyên
    std::optional<int> max_value_;  // Thêm mới: giá trị lớn nhất của số nguyên
...
public:
    ...
    inline void set_value(const T& value) {
        // Thêm kiểm tra phạm vi cho giá trị số nguyên được thiết lập
