...
bool AudioCodec::SetOutputSampleRate(int sample_rate) {
    // Xử lý đặc biệt: nếu truyền vào -1, có nghĩa là đặt lại về tỷ lệ lấy mẫu gốc
    if (sample_rate == -1) {
        if (original_output_sample_rate_ > 0) {
    ...
    // Trước tiên thử vô hiệu hóa kênh I2S (nếu đã được bật)
    ...
    if (disable_ret == ESP_OK) {
        ...
        // Kênh có thể đã ở trạng thái bị vô hiệu hóa, điều này là bình thường
    ...
    // Bật lại kênh (bất kể trạng thái trước đó, bây giờ cần bật để phát âm thanh)
