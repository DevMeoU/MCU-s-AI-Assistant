...
bool AudioCodec::SetOutputSampleRate(int sample_rate) {
    // Xử lý đặc biệt: Nếu truyền vào -1, có nghĩa là đặt lại về tỷ lệ mẫu gốc
    if (sample_rate == -1) {
        if (original_output_sample_rate_ > 0) {
    ...
    // Bật lại kênh (bất kể trạng thái trước đó, lúc này cần bật để phát âm thanh)
